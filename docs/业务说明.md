...
多人会议画面区采用 **最多 3 列** 网格，窗口按从左到右、自上而下依次排列，每个窗口保持 **16:9** 比例，并根据人数自动缩小以适配可视区域（无需滚动页面即可看到底部工具栏）。

| 人数 | 排列示例 |
|------|----------|
| 3 人 | 第一行 3 个窗口 |
| 4 人 | 第一行 3 个 + 第二行 1 个 |
| 7 人 | 第一行 3 个 + 第二行 3 个 + 第三行 1 个 |

#### 2.3 窗口放大（双击切换）

双击任意参会者窗口可进入**聚焦模式**；再次双击当前大窗口可恢复默认网格。

| 操作 | 效果 |
|------|------|
| 双击某个窗口 | 该窗口放大至左侧主区域，其余窗口缩小为右侧缩略图 |
| 双击当前大窗口 | 恢复所有窗口等大的默认网格 |
| 双击右侧某个缩略图 | 与该缩略图交换：被双击者放大，原大窗口缩至右侧 |

聚焦模式布局要点：

- **大窗口**：占据左侧剩余宽度，在可视区域内尽可能放大；左右保留边距，无需滚动。
- **小窗口**：保持 16:9，在右侧**自上而下紧凑排列**，相邻小窗间距较小，底部留白较大。
- 仅 1 个窗口时双击无效；对端离开导致窗口消失时，自动退出聚焦状态。
...
| 布局 | 默认对端全屏、本端右下角小窗（`call-layout`）；支持双击切换大小窗 |
...
| 状态 | 效果 |
|------|------|
| 默认 | 对端画面全屏，本端摄像头缩至右下角小窗 |
| 双击任意一方窗口 | 被双击者全屏，另一方缩至右下角小窗 |
| 仅本端在房间 | 对端未接入，双击无效 |
...
- 业务选择首页（2×2 卡片布局）+ 弹窗表单（昵称/房间号/流名按业务显隐）
...
- **会议**：默认 3 列 16:9 网格；双击窗口放大/还原，聚焦模式下右侧缩略图自上而下紧凑排列
- **1v1**：默认对端全屏 + 本端右下角小窗；双击任意窗口可切换大小窗
...
界面截图见项目 [README](../README.md#界面预览)。

| 业务 | 默认布局 | 双击切换 |
|------|----------|----------|
| 多人会议 | 最多 3 列 16:9 网格，自动适配人数与视口 | 放大左侧主画面 + 右侧缩略图栈（见上文 **2.2 / 2.3**） |
| 1v1 通话 | 对端全屏 + 本端右下角小窗 | 双击切换本端/对端大小窗（见上文 **3.1**） |
